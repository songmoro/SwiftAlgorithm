...
func solution(_ today:String, _ terms:[String], _ privacies:[String]) -> [Int] {
    // MARK: 오늘 날짜를 기준으로 개인정보가 수집된 날짜가 약관에 명시된 날짜를 넘어서는지 검사
    var answer: [Int] = []
    var parsedTerms: [String: Int] = [:]
    ...
    terms.forEach { term in
        ...
        parsedTerms[split[0]] = Int(split[1])
    }
    
    for (index, privacy) in privacies.enumerated() {
        let split = privacy.split(separator: " ").map { String($0) }
        let month = DateComponents(month: Int(parsedTerms[split[1]]!))
        
        let date = dateFormatter.date(from: split[0])!
